...
public class GameMoveAction implements GameAction {
	...
	public void read(DofusStream stream) {
		...
		for (int i = 0; i < data.length(); i += 3) {
			path.put(Compressor.uncompressCellId(data.substring(i + 1, i + 3)), PathDirection.valueOf(Crypt.indexOfHash(data.charAt(i))));
		}
	}
	...
	public void write(DofusStream stream) {
		...
		for (Map.Entry<Integer, PathDirection> point : path.entrySet())
			sb.append(Crypt.hashToIndex(point.getValue().ordinal())).append(Compressor.compressCellId(point.getKey()));
